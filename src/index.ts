...
type CdnUrlMode = "image" | "video" | "poster" | "gif" | "origin";

function appendCdnQuery(search: URLSearchParams, params: Record<string, string | number | boolean | undefined | null>) {
...
function buildCdnUrl(input: {
  ...
  if (input.mode === "gif") params.format = "gif";
...
function resolveAllowed(input: string) {
  ...
  if (!allowed) {
    ...
    throw new Error(
      `Path '${resolved}' is outside Convertly MCP's approved roots. ` +
...
server.registerTool(
  "build_cdn_url",
  {
    ...
    description:
      "Build a Convertly CDN URL for image transforms, video transcode/clip, poster frames, GIFs, or origin-backed assets. " +
    ...
    inputSchema: {
      ...
      params: z
        ...
        .optional()
        .describe(
          "Transform query params — e.g. { w: 800, format: 'webp', trim: true } or { format: 'mp4', so: 0, du: 30, mute: true }",
        ),
    },
  },
  async ({ deliveryKey, fileId, mode, baseUrl: cdnBaseUrl, originSlug, originPath, params }) =>
    jsonResult({
      url: buildCdnUrl({
        deliveryKey,
        fileId,
...
server.registerTool(
  "convert_media",
  {
    title: "Convert Media",
    description:
      "Re-encode local image, video, or audio files to a different format via Convertly. Any-to-any within these sets:\n" +
...
server.registerTool(
  "inspect_media",
  {
    title: "Inspect Media",
    description:
      "Return dimensions, duration, format, and other metadata for local media files. " +
...
type ConditionGroupShape = {
  ...
  logic: "and" | "or";
  negate?: boolean;
  conditions: Array<{ id: string; field: z.infer<typeof conditionFieldEnum>; operator: z.infer<typeof conditionOperatorEnum>; value?: string; negate?: boolean }>;
...
server.registerTool(
  "create_workflow",
  {
    ...
    description:
      "Create a new Convertly workflow. A workflow is a reusable pipeline that runs whenever you call run_workflow with an input file. " +
...
server.registerTool(
  ...
  async ({ id, confirm }) => {
    if (!confirm) return jsonResult({ dryRun: true, wouldDelete: id, note: "Call again with confirm=true to delete." });
...
server.registerTool(
  "run_workflow",
  {
    title: "Run Workflow",
    description:
      "Run a saved workflow against a local file. Uploads the file to Convertly, executes every step in the workflow definition, and returns the run record. " +
