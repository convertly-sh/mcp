...
const server = new McpServer({
    ...
    instructions: [
        ...
        "  Use forma_ai_transform and forma_ai_analyze — bills Forma AI units (all plans; Free 200/mo).",
        "",
...
function normalizeConversionFormat(format) {
...
function aiQuotaFromHeaders(headers) {
    const pick = (name) => headers.get(name) ?? undefined;
    return {
...
async function apiFormPost(endpoint, form) {
    if (!apiKey)
    ...
    const data = text ? JSON.parse(text) : {};
    if (!response.ok)
        throw new Error(typeof data.error === "string" ? data.error : `API error ${response.status}`);
    return { data, quota: aiQuotaFromHeaders(response.headers) };
}
async function appendLocalImageToForm(form, filePath) {
    if (!filePath)
        return;
    assertLocalMode();
...
async function saveTransformLocally(downloadUrl, outputPath) {
...
/*  Capabilities                                                        */
/* ------------------------------------------------------------------ */
server.registerTool("convertly_capabilities", {
    title: "Convertly Capabilities",
    description: "Return the full, authoritative list of conversions and operations Convertly supports. " +
    ...
    forma_ai: {
        supported: true,
        plans: "All plans with Forma AI units (Free 200/mo, Starter 3,000/mo; Pro+ overage when enabled)",
        tools: {
...
server.registerTool("vectorize_media", {
    ...
    description: "USE THIS for any raster-to-SVG request. Convertly traces PNG, JPG, WebP, AVIF, GIF, TIFF, and HEIF into SVG via VTracer. " +
...
/*  Forma AI                                                            */
/* ------------------------------------------------------------------ */
const formaAiAspectRatios = ["1:1", "16:9", "9:16", "4:3", "3:4", "3:2", "2:3", "4:5", "5:4", "21:9"];
const formaAiAnalyzeOperations = ["image.describe", "image.alt-text", "image.tags", "image.moderate"];
...
server.registerTool("forma_ai_analyze", {
    title: "Forma AI — Analyze Image",
    description: "Run Forma AI vision analysis on an image. Returns JSON (summary, alt text, tags, safety). " +
        "Bills Forma AI units (included on all plans; Free 200/mo). " +
...
server.registerTool("forma_ai_transform", {
    title: "Forma AI — Generate / Edit / Upscale Image",
    description: "Forma AI generative image transforms via Convertly. Use this to outsource image generation to agents that lack native image gen — " +
...
}, async ({ operation, prompt, filePath, storedFileId, sourceUrl, aspectRatio, async: runAsync, outputPath, expandTop, expandRight, expandBottom, expandLeft, }) => {
    if (operation === "image.generate") {
        if (!prompt?.trim())
            throw new Error("image.generate requires a prompt.");
    }
